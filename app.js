// ============================================
// КЛАСС ПРИЛОЖЕНИЯ НАВИГАЦИИ 1430
// ============================================
// Основной класс, отвечающий за всю логику навигационной системы
// Управление картой, маршрутами, QR-кодами, голосом и ролями
// ============================================
...
class SchoolNavigationApp {
  ...
  // ============================================
  // Запускает все основные компоненты: canvas, события, голос, данные
  // ============================================
  ...
  // ============================================
  // НАСТРОЙКА CANVAS ДЛЯ ОТРИСОВКИ
  // ============================================
  // Конфигурация холста с учётом DPI дисплея
  // Автоматическое изменение размеров при ресайзе
  // ============================================
  ...
  // ============================================
  // Клики, клавиатура, тач-события, модальные окна
  // Переключение этажей, режимов, кнопок управления
  // ============================================
  ...
  // ============================================
  // НАСТРОЙКА ГОЛОСОВОЙ НАВИГАЦИИ
  // ============================================
  // Инициализация Web Speech API
  // Поиск русских голосов, настройки скорости/тона
  // ============================================
  ...
  // ============================================
  // Создание примера карты школы № 1430
  // 3 этажа, кабинеты, лестницы, коридоры и выходы
  // ============================================
  ...
  // Учёт межэтажные переходы через лестницы
  // ============================================
  ...
  // ============================================
  // Преобразование пикселей canvas в метры (реальное расстояние)
  // Конфигурируемые масштабы, цветовая кодировка
  // ============================================
  ...
  // ============================================
  // ГОЛОСОВЫЕ ИНСТРУКЦИИ ПО МАРШРУТУ
  // ============================================
  // Генерация пошаговых голосовых подсказок
  // Указание направлений, расстояний и этажей
  // ============================================
  ...
  // ============================================
  // QR-СКАНЕР ДЛЯ ПОЗИЦИОНИРОВАНИЯ
  // ============================================
  // Активация камеры для сканирования QR-кодов на объектах
  // Обработка ошибок камеры, повторные попытки
  // ============================================
  ...
  // ============================================
  // СОЗДАНИЕ И СКАЧИВАНИЕ README.TXT
  // ============================================
  // Генерация текстового файла с описанием проекта
  // ============================================
  
  downloadReadme() {
    const readmeContent = `НАВИГАЦИЯ 1430 - СИСТЕМА НАВИГАЦИИ ПО ШКОЛЕ
=========================================================

О ПРОЕКТЕ:
Веб-приложение для интерактивной навигации по школьному зданию.
Позволяет строить маршруты, сканировать QR-коды,
получать голосовые подсказки и управлять картой.
...
1. НАВИГАЦИЯ:
   ...
   • Цветовая кодировка сегментов (короткие/средние/длинные)
   • Межэтажные переходы через лестницы

2. ГОЛОСОВЫЕ ПОДСКАЗКИ:
   • Пошаговые инструкции на русском языке
   • Настраиваемая скорость и тон речи
   • Выбор голоса синтезатора
   • Указание общей длины маршрута

3. QR-ПОЗИЦИОНИРОВАНИЕ:
   • Сканирование QR-кодов через камеру
   • Определение текущего местоположения
   • Автоматическое переключение на нужный этаж
   • Подсветка текущей позиции на карте

4. КАРТОГРАФИЯ И РЕДАКТИРОВАНИЕ:
   • Послойное отображение (до 3-х этажей)
   • Масштабирование и панорамирование
   • Загрузка фоновых схем этажей (только админ)
   • Добавление/редактирование объектов

5. КЛАССИФИКАЦИЯ ОБЪЕКТОВ:
   • Кабинеты (квадрат, синий)
   • Лестницы (треугольник, зелёный)
   • Коридоры (прямоугольник, фиолетовый)
   • Выходы (пятиугольник, красный)

РОЛИ ПОЛЬЗОВАТЕЛЕЙ:

1. УЧЕНИК:
   • Просмотр карты (без редактирования)
   • Построение маршрутов и навигация
   • QR-сканирование и позиционирование
   • Настройка голосовых подсказок
   • Загрузка сохранённых карт

2. АДМИНИСТРАТОР (пароль: 1430):
   • Все функции ученика
   • Создание и редактирование карты
   • Добавление новых этажей
   • Загрузка фоновых схем
   • Сохранение карты в JSON
   • Очистка всех данных

МОБИЛЬНОЕ ПРИЛОЖЕНИЕ:
• Адаптивный дизайн для планшетов и смартфонов
• На экранах <768px скрыты кнопки сохранения/загрузки
• Оставлена только кнопка смены роли
• Полноэкранное выдвижное меню
• Тач-события для работы пальцами

ТЕХНИЧЕСКИЕ ДАННЫЕ:

Архитектура: Vanilla JavaScript + HTML5 Canvas + CSS3
Браузерная поддержка: Chrome 80+, Firefox 75+, Safari 13+
Минимальное разрешение: 320x568px (iPhone SE)
Оптимальное разрешение: 1280x720px+ (десктоп)

Используемые API:
• HTML5 Canvas — отрисовка карты
• Web Speech API — синтез речи
• MediaDevices.getUserMedia() — доступ к камере
• File API — сохранение/загрузка файлов
• Html5Qrcode library — распознавание QR-кодов
...
• Используется алгоритм Дейкстры (Dijkstra's algorithm)
• Находит кратчайший путь между двумя точками
• Учитывает межэтажные переходы
...
БЕЗОПАСНОСТЬ:
• Нет серверного компонента - всё работает локально
• Пароль администратора зашит в коде (1430)
• Не сохраняет персональные данные
• Камера используется только для QR-сканирования
• Нет внешних сетевых запросов (кроме библиотек)

ОПТИМИЗАЦИЯ:
• Быстрая отрисовка через requestAnimationFrame
• Оптимизация для разных DPI экранов
• Малый размер (менее 200KB без изображений)
• Нет внешних зависимостей (кроме QR библиотеки)
• Progressive Web App возможности

ИСПОЛЬЗОВАНИЕ:

1. Откройте index.html в современном браузере
2. Выберите свою роль: Ученик или Администратор
3. Для администратора введите пароль: 1430
4. Начните работу с картой!

КОНТАКТЫ:
Проект разработан для школы № 1430
Последнее обновление: Октябрь 2025`;
    
    const dataUri = 'data:text/plain;charset=utf-8,' + encodeURIComponent(readmeContent);
    const link = document.createElement('a');
    link.href = dataUri;
    link.download = 'README.txt';
    link.click();
    
    this.showNotification('README.txt скачан', 'success');
  }
  ...
  // ============================================
  // ОТРИСОВКА КАРТЫ НА CANVAS
  // ============================================
  // Основной цикл отрисовки: фон, связи, узлы, маршрут
  // Подсветка активных элементов, анимации
  // ============================================
  ...
  // ============================================
  // Разделение доступа: Ученик (навигация) / Администратор (редактирование)
  // Пароль для админ-функций: '1430'
  // ============================================
...
// ============================================
// Автоматический запуск при готовности DOM
// Создание экземпляра класса и экспорт в глобальную область
// ============================================
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log('School Navigation App v3.0 - Навигация 1430 - initialized successfully');
});